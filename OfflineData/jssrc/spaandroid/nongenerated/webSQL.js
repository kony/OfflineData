...
function createTable(dbId) {
    ...
    ((insertTable) != null) && i <= insertTable.length; i++) {
        var v = insertTable[kony.decrement(i)]; //kony.decrement
...
function success_sqlSelect(transactionId, resultset) {
    if (resultset != null) {
        ...
        for (var i = numResults - 3; i <= numResults - 1; i++) {
            var rowItem = kony.db.sqlResultsetRowItem(transactionId, resultset, i);
            var addItem = {
                lblEmpID2: rowItem.empid.toString(),
                lblEmpName2: rowItem.empname,
                lblDepId2: rowItem.depid.toString()
            };
            insertTable.push(addItem);
        }
...
function doTransactionsqlUpdate() {
    kony.db.transaction(baseObjectId, sqlSelect_Update, commonErrorCallback, commonVoidcallback);
}
/*****************************************************************
...
function sqlSelect_Update(dbId) {
    try {
        var sqlStatement = "SELECT * FROM emp_details";
        kony.db.executeSql(dbId, sqlStatement, null, success_sqlSelect_Update, commonErrorCallback);
    } catch (err) {
        kony.print("error while selecting " + err);
    }
}
...
function doTransactionsqlDelete() {
    kony.db.transaction(baseObjectId, sqlSelect_Delete, commonErrorCallback, commonVoidcallback);
}
/*****************************************************************
...
function sqlSelect_Delete(dbId) {
    try {
        var sqlStatement = "SELECT * FROM emp_details";
        kony.db.executeSql(dbId, sqlStatement, null, success_sqlSelect_Delete, commonErrorCallback);
    } catch (err) {
        kony.print("error while selecting " + err);
    }
}
